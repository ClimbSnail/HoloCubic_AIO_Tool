...
class FileManager(object):
    ...
    def init_path_tree(self, father):
        ...
        # <<TreeviewOpen>>
        # <<TreeviewClose>>
        # <Button-1> <Button-2> <Button-3> 鼠标左、右键单机
        self.tree.bind("<<TreeviewOpen>>", lambda event: self.tree_open())
        self.tree.bind("<Button-3>", lambda event: self.tree_button_3())
    ...
    def reflush_folder(self, updata_path, sub_file_list):
        ...
        try:
            ...
            for sub_file_name in sub_file_list:
                sub_tmp = None
                if "/" == sub_file_name[-1]:
                    sub_tmp = {"tree":None, "type":"folder", "name":sub_file_name[:-1], "path":updata_path+sub_file_name[:-1], "sub_file":[]}
                else:
    ...
    def tree_button_3(self):
        """
        Tree元素被右键单击
        """
        try:
            # for item in self.tree.focus():
            for item in self.tree.selection():
                # self.tree.focus()
                print(item)
